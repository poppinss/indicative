...
group('Raw | Types', function() {
  ...
  test('return false when input is a boolean and strict is enabled', function(assert) {
    assert.equal(Is.isNumber(true, true), false)
  })

  test('return false when input is a boolean and strict is enabled', function(assert) {
    assert.equal(Is.isNumber(false, true), false)
  })
